    ...
    switch call.method {
      case "prepareToRecordMpeg4Aac":
        ...
        do {
          ...
          result(FlutterError(code: "prepareToRecordMpeg4Aac", message: "Failed to create audio recording file", details: "\(error)"))
          return
        }
        ...
        audioMixerNode!.installTap(onBus: 0, bufferSize: 1024, format: audioMixerNode!.outputFormat(forBus: 0), block: {
          ...
          vDSP_meamgv(samples, 1, &avgAmp, UInt(buffer.frameLength))
          vDSP_maxmgv(samples, 1, &maxAmp, UInt(buffer.frameLength))
          // var v:Float = -100
          // if avgValue != 0 {
        ...
        self.eventSink?([
          "type": "audioRecorder/prepared",
          "payload": [
            "url": audioRecordingFile!.url.absoluteString,
          ],
        ])
      ...
      case "deleteRecording":
        do {
          ...
          self.eventSink?([
            "type": "audioRecorder/deletedRecording",
            "payload": [
              "url": audioRecordingFile!.url.absoluteString,
            ],
          ])
